...
var bgscale;
$('#img-width').on("input change", function() {
            ...
            lsvgobj.clone(function(clone) {
                ...
                $.ajax({
                    ...
                    success: function(msg) {
                        ...
                        if (isupdatetemplate) updateTemplate();
                    }
                })
            });
        }
...
function getTextcategory() {
    $.ajax({
        ...
        url: "getextategory",
        success: function(data) {
            $("#textcat-select2").empty();
            $("#textcat-select2").html(data);
            //$(".child_menu .dropdown-content.select-dropdown").html(data);
            //$("#textcat-select2").prev().html(data);
            //$("#text_category").empty();
...
function togglethisgetTexts(id,texid) {
    $('#'+id).toggle();
    getTexts(texid);
}
    ...
    WebFont.load({
        ...
        active: function() {
            ...
            if (isSave) {
                ...
                //save template to server and insert template id into db.
                canvasarray[0].renderAll();
                var pngdataURL = canvasarray[0].toDataURL("image/png");
                var categoryId = '';
                var path = "templates";
                filename = filename.replace('.ype', '');
...
$(document).ready(function() {
    ...
    $("#dropdown5 li").on("click", function(e) {
        //alert('success');
...
function getTemplates(id, refresh) {
    ...
    $.ajax({
        type: 'GET',
        url: 'get-templates?offset=' + tempoffset + '&limit=12&tempid=' + tempid + '&refresh=' + refresh,
        success: function(data) {
            if (data != '') {
...
function getbgimages(id) {
    var IsBgselected = IsBgselected || '';
    var bgcatId = id || '';
    if (IsBgselected == false) {
        var lastbackground = $('.backgroundGallery').children().last().attr('id');
    ...
    $.ajax({
        type: 'GET',
        url: 'get-bgimages?offset=' + bgoffset + '&limit=12&category=' + bgcatId,
        success: function(data) {
            if (data != '') {
                $('.backgroundGallery').append(data);
                IsBgselected = false;
            } else {
                bgoffset = 0;
            }
...
function getcatimages(id) {
    var Iscatselected = Iscatselected || '';
    if (typeof id != 'undefined') {
        var catId = id;
    } else {
        var catId = '';
    ...
    $.ajax({
        type: 'GET',
        url: 'get-catimages?offset=' + catoffset + '&limit=12&category=' + catId,
        success: function(data) {
            if (data != '') {
...
function getTexts(id) {
    ...
    $.ajax({
        type: 'GET',
        url: 'get_texts?offset=' + textoffset + '&limit=12&textid=' + textid,
        success: function(data) {
            if (data != '') {
                ...
                var responsedata = data;
                $('#text_container').empty();
                $('#text_container').html(responsedata);
